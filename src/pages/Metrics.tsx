    ...
    return (
      <AuthGuard>
        <div className="bg-app">
          <BackgroundFX />
          <Navbar />
          <main className="relative z-10 container-page py-8 md:py-10 flex-1 flex items-center justify-center">
            <div className="card text-center max-w-lg">
              <div className="card-body">
                <span className="text-4xl">🚀</span>
                <h2 className="text-2xl font-bold mt-2">Funcionalidade PRO</h2>
                <p className="text-slate-600 mt-1">A página de Métricas é um recurso exclusivo para assinantes do plano PRO.</p>
                <Link to="/plans" className="btn-primary mt-4">Conheça os Planos e Faça o Upgrade</Link>
              </div>
            </div>
          </main>
          <Footer />
...
function MetricsPageContent() {
  const [view, setView] = useState<'total' | 'mensal'>('total');
  ...
  const paymentsVisible = useMemo(() => allPayments.filter((p) => !p.deleted && debtorById[p.debtorId!]), [allPayments, debtorById]);
  ...
  const pieData = useMemo(() => { 
    const data = view === 'total' 
      ? [{ name: 'Pago', value: stats.pagoTotal }, { name: 'Pendente', value: stats.pendente }] 
      : [{ name: 'Recebido no mês', value: stats.pagoMensal }, { name: 'Pendente', value: stats.pendente }];
    if (stats.totalDevedores > 0) { data.push({ name: 'Devedores', value: stats.totalDevedores }); }
    return data; 
  }, [view, stats]);
  ...
  function downloadCsv() {
    ...
    for (const d of debtors) {
      ...
      let totalRemaining = Number(d.total || 0);

      if (plist.length === 0) {
        lines.push([q(d.name), q(formatMoneyBR(totalRemaining)), q(createdBr), '', '', '', '', '0', '0', '0'].join(SEP));
      } else {
        for (const p of plist) {
          const payBr = p.date ? new Date(p.date).toLocaleString('pt-BR') : '';
          const valNum = Number(p.amount || 0);
          const valFmt = formatMoneyBR(valNum);
          ...
          const agendado = p.scheduled ? '1' : '0';
          const pago = p.paid ? '1' : '0';
          const pendente = Math.max(0, totalRemaining - valNum);

          lines.push([
            q(d.name),
            q(formatMoneyBR(totalRemaining)),
            q(createdBr),
            q(valFmt),
            q(payBr),
            q(methodStr),
            ...
            agendado,
            pago,
            formatMoneyBR(pendente)
          ].join(SEP));

          totalRemaining -= valNum;
        }
      }
    }
        ...
        <main className="relative z-10 container-page py-8 md:py-10 flex-1 space-y-6">
          <div className="flex items-end justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="muted">Visão {view === 'total' ? 'consolidada' : 'do mês atual'}.</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="inline-flex rounded-xl border border-emerald-200 bg-white p-1">
                <button type="button" className={`px-3 py-1.5 text-sm rounded-lg transition ${view === 'total' ? 'bg-emerald-100 text-emerald-800 font-semibold' : 'text-slate-700'}`} onClick={() => setView('total')}>TOTAL</button>
                <button type="button" className={`px-3 py-1.5 text-sm rounded-lg transition ${view === 'mensal' ? 'bg-emerald-100 text-emerald-800 font-semibold' : 'text-slate-700'}`} onClick={() => setView('mensal')}>MENSAL</button>
              </div>
              <button onClick={downloadCsv} className="btn-soft whitespace-nowrap">BAIXAR EM PLANILHA</button>
            </div>
          </div>

          {loading && <div className="card"><div className="card-body text-center text-slate-500">Calculando métricas...</div></div>}

          {!loading && <>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="card"><div className="card-body"><div className="text-sm text-slate-600">Total de devedores</div><div className="text-2xl font-extrabold" style={{ color: COLORS_BY_NAME['Devedores'] }}>{stats.totalDevedores}</div></div></div>
              <div className="card"><div className="card-body"><div className="text-sm text-slate-600">Total devido</div><div className="text-2xl font-extrabold text-slate-800">R$ {formatMoneyBR(stats.totalDevido)}</div></div></div>
              <div className="card"><div className="card-body"><div className="text-sm text-slate-600">{view === 'mensal' ? 'Pago no mês' : 'Total pago'}</div><div className="text-2xl font-extrabold" style={{ color: COLORS_BY_NAME['Pago'] }}> R$ {formatMoneyBR(view === 'mensal' ? stats.pagoMensal : stats.pagoTotal)} </div></div></div>
              <div className="card"><div className="card-body"><div className="text-sm text-slate-600">Pendente (geral)</div><div className="text-2xl font-extrabold" style={{ color: COLORS_BY_NAME['Pendente'] }}>R$ {formatMoneyBR(stats.pendente)}</div></div></div>
            </div>

            <div className="card"><div className="card-body" style={{ height: 380 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" labelLine={false} outerRadius={120} dataKey="value">
                    {pieData.map((entry) => (<Cell key={entry.name} fill={COLORS_BY_NAME[entry.name]} />))}
                  </Pie>
            ...
            <div className="card"><div className="card-body">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold">{view === 'mensal' ? 'Recebido no mês por devedor' : 'Recebido (acumulado) por devedor'}</h3>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-slate-600 border-b">
                      ...
                      <th className="py-2 text-right">{view === 'mensal' ? 'Recebido no mês' : 'Recebido (ac.)'}</th>
                    </tr>
                  </thead>
                  <tbody>
